...
import { UserPlus, Target, PlaySquare, Coins, Rocket, HelpCircle } from 'lucide-react';
...
export default function HowItWorks({ section }: HowItWorksProps) {
  const stepsFromDb = section?.content?.steps;
  const hasDbSteps = Array.isArray(stepsFromDb) && stepsFromDb.length > 0;
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-20">
          ...
          <p className="text-white/60 text-lg max-w-2xl mx-auto">
            {section?.subtitle || 'A simple, transparent ecosystem designed to generate real growth and real rewards.'}
          </p>
        </div>
        ...
        <div className="relative max-w-5xl mx-auto pt-4">
          ...
          <div className={`grid grid-cols-1 gap-8 ${hasDbSteps ? `md:grid-cols-${Math.min(5, stepsFromDb.length)}` : 'md:grid-cols-5'}`}>
            {hasDbSteps ? (
              stepsFromDb.map((step: any, i: number) => (
                <motion.div 
                  key={i}
                ...
                >
                  ...
                  <div className="w-16 h-16 rounded-2xl bg-[#0A1128] border border-white/10 flex items-center justify-center mb-6 relative z-10 group-hover:scale-110 group-hover:border-primary/50 transition-all duration-300 shadow-xl group-hover:shadow-primary/20">
                    <HelpCircle className="w-8 h-8 text-white/70 group-hover:text-primary transition-colors" />
                    {/* Step Number Badge */}
