...
export default function HeroSection({ section }: HeroSectionProps) {
  const ctaText = section?.content?.ctaText || 'Start Growing';
  const ctaLink = section?.content?.ctaLink || '/register';
  const secondaryCtaText = section?.content?.secondaryCtaText || 'Explore Tasks';
  const secondaryCtaLink = section?.content?.secondaryCtaLink || '/tasks';
  const customHeroImage = section?.content?.heroImage;
      ...
      <div className="container relative z-10 mx-auto px-4 grid lg:grid-cols-2 gap-16 items-center">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-white/60 leading-relaxed mb-10 max-w-xl">
            {section?.subtitle || 'Connect creators and viewers through rewarded engagement tasks that drive real growth, authentic watch-time, and massive ROI.'}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Link href={secondaryCtaLink}>
              <Button size="lg" variant="outline" className="w-full sm:w-auto h-14 px-8 text-lg bg-white/5 border-white/10 hover:bg-white/10 backdrop-blur-md text-white rounded-xl">
                {secondaryCtaText} <Play className="ml-2 w-5 h-5" />
              </Button>
            </Link>
          </div>
        ...
        >
          {customHeroImage ? (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[550px] rounded-3xl overflow-hidden border border-white/10 shadow-2xl bg-[#0A1128]/80 backdrop-blur-xl p-2">
              <img src={customHeroImage} alt="Hero Banner" className="w-full h-auto rounded-2xl object-cover" />
            </div>
          ) : (
            <>
              {/* Main Dashboard Card */}
